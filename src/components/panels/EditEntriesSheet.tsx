...
export function EditEntriesSheet({
  ...
  const [draftProtocolNumber, setDraftProtocolNumber] = useState("");
  ...
  const liveZScore = Number.isFinite(draftODNumber)
    ? calculateZScore(draftODNumber, mean, sd)
    : null;

  useEffect(() => {
    if (!open) {
      setExpandedEntryId(null);
      setDraftProtocolNumber("");
      setDraftODValue("");
      setEditReason("");
      setInlineError(null);
      setIsSaving(false);
  ...
  const startEditing = (entry: QCEntry) => {
    ...
    setDraftProtocolNumber(entry.protocolNumber);
  ...
  const handleSave = async () => {
    ...
    const trimmedProtocolNumber = draftProtocolNumber.trim();

    if (!trimmedProtocolNumber) {
      setInlineError("Protocol No. is required.");
      return;
    }

    if (
      !Number.isFinite(draftODNumber) ||
      draftODNumber < 0 ||
      draftODNumber > 10
    ) {
    ...
    try {
      ...
      setInlineError(
        error instanceof Error
          ? error.message
          : "Unable to save this QC entry change.",
      );
    } finally {
      setIsSaving(false);
    }
                      ...
                      {isSigned ? (
                        ...
                        >
                          ...
                          <span className="sr-only">
                            Edit entry {entry.protocolNumber}
                          </span>
                        </Button>
                      )}
                    </div>
                      ...
                      <div className="mt-4 rounded-lg border border-[#E5EAF2] bg-[#F9FBFF] p-4">
                        <div className="space-y-4">
                          <div className="space-y-2">
                            ...
                            <input
                              type="text"
                              value={draftProtocolNumber}
                              onChange={(event) =>
                                setDraftProtocolNumber(event.target.value)
                              }
                              placeholder="e.g. MR-2026-06-001"
                          ...
                          <div className="space-y-2">
                            ...
                            <input
                              ...
                              value={draftODValue}
                              onChange={(event) =>
                                setDraftODValue(event.target.value)
                              }
                              className="w-full rounded-lg border border-[#DCE4F2] px-3 py-2 text-sm outline-none transition focus:border-[#0000FF]"
                            />
                            <p
                              ...
                              style={{
                                color:
                                  liveZScore === null
                                    ? "#64748B"
                                    : getZScoreTone(liveZScore),
                              }}
                            >
                              {liveZScore === null
                                ? "Z-score updates after a valid OD value is entered."
                                : `z = ${liveZScore.toFixed(2)}`}
                            </p>
                          </div>
