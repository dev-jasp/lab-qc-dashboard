...
export function QCRulesReferenceCard({
  className = "",
  minRunsForWestgard,
}: QCRulesReferenceCardProps) {
  return (
    <section
      className={`qc-card ${className}`.trim()}
      aria-labelledby="qc-rules-reference-heading"
    >
      <div className="mb-5 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div className="max-w-2xl">
          <h2
            id="qc-rules-reference-heading"
            className="text-[16px] font-semibold text-[#111827]"
          >
            Westgard QC Rules
          </h2>
          <p className="mt-2 text-[13px] leading-6 text-[#6b7280]">
            Complete rules list used by this monitor. Evaluation begins after{" "}
            {minRunsForWestgard} control runs.
          </p>
        </div>
          ...
          return (
            ...
            >
              <div className="mb-3 flex items-start justify-between gap-3">
                ...
                <span
                  className={`shrink-0 rounded-full px-2.5 py-1 text-[11px] font-bold ${severityMeta.badgeClassName}`}
                >
                  {severityMeta.label}
                </span>
              </div>
